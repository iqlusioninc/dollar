...
func TestCircuitBreakerActivation(t *testing.T) {
	...
	// Try to recalculate AUM with a 20% increase (should trigger circuit breaker)
	...
	assert.Equal(t, sdkmath.NewInt(1000*ONE_V2), trip.PreviousAum)
	assert.Equal(t, sdkmath.NewInt(2000*ONE_V2), trip.AttemptedAum)
...
func TestCircuitBreakerNotActivatedWithinLimit(t *testing.T) {
	...
	// Set AUM change limit to 20% (2000 bps)
	params, err := k.GetVaultsV2Params(baseCtx)
	require.NoError(t, err)
	params.MaxAumChangeBps = 2000
	require.NoError(t, k.SetVaultsV2Params(baseCtx, params))
...
func TestCircuitBreakerMultipleTrips(t *testing.T) {
	...
	// Set AUM change limit to 5% (500 bps)
	params, err := k.GetVaultsV2Params(baseCtx)
	require.NoError(t, err)
	params.MaxAumChangeBps = 500
	require.NoError(t, k.SetVaultsV2Params(baseCtx, params))

	// First trip
	aumInfo1 := vaultsv2.AUMInfo{
		CurrentAum:  sdkmath.NewInt(1000 * ONE_V2),
		PreviousAum: sdkmath.ZeroInt(),
		LastUpdate:  time.Now(),
	}
	require.NoError(t, k.SetVaultsV2AUMInfo(baseCtx, aumInfo1))
	...
	require.NoError(t, k.SetVaultsV2AUMInfo(baseCtx, aumInfo2))
...
func TestCircuitBreakerWithNegativeChange(t *testing.T) {
	...
	assert.Equal(t, sdkmath.NewInt(2000*ONE_V2), trip.PreviousAum)
	assert.Equal(t, sdkmath.NewInt(1500*ONE_V2), trip.AttemptedAum)
}
...
func TestCircuitBreakerWithRealOracleFlow(t *testing.T) {
	...
	params, err := k.GetVaultsV2Params(baseCtx)
	require.NoError(t, err)
	params.MaxAumChangeBps = 1500 // 15%
	require.NoError(t, k.SetVaultsV2Params(baseCtx, params))
	...
	// Create AUM message with 20% increase: 800 * 1.2 = 960
	...
	assert.True(t, found)
	assert.Equal(t, positionID, trip.RemotePositionId)
	// New AUM would be: 200 local + 960 remote = 1160
	// Previous AUM: 1000
	// Change: 16% = 1600 bps > 1500 bps limit
	assert.Equal(t, int32(1600), trip.ChangeBps)
}

// Helper function to create mock AUM payload
func createMockAUMPayload(positionID uint64, sharePrice sdkmath.LegacyDec, sharesHeld sdkmath.Int) []byte {
